...
client.on('message', (message) => {
  ...
  if(message.content == '!bi') {
    ...
    embed.addField('사용자수',         `${client.users.size.toLocaleString()}`, true);
    embed.addField('소속서버',       `${client.guilds.size.toLocaleString()}`, true);
  ...
  if(message.content == 'embed') {
    ...
    let embed = new Discord.RichEmbed()
      .setTitle('디스코드')
      .setURL('http://promise.r-e.kr/')
      .setAuthor('PromiseDevTeam', img, 'http://promise.r-e.kr/')
      .setThumbnail(img)
      .addBlankField()
      .addField('PromiseDevTeam', '임베드예시입니다.')
      .addField('아래로도', '더 추가 가능합니다.', true)
      .addBlankField()
      .setTimestamp()
    ...
    let commandStr = '';
    let embed = new Discord.RichEmbed()
      .setAuthor('PromiseDevTeam 명령어', helpImg)
      .setColor('#186de6')
      .setFooter(`PromiseDevTeam`)
      .setTimestamp()
    ...
    if(message.member != null) { // 채널에서 공지 쓸 때
      let contents = message.content.slice('!전체디엠'.length);
      let embed = new Discord.RichEmbed()
        .setAuthor('PromiseDevTeam공지')
        .setColor('9d1be8')
        .setFooter(`PromiseDevTeam`)
        .setTimestamp()
  
      embed.addField('소제목', contents);
